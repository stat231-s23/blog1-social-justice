...
`Number of Articles` <- c(as.numeric(count(policy)), as.numeric(count(health)), as.numeric(count(finances)), as.numeric(count(politics)), as.numeric(count(sports)))
title_topics <- data.frame(topics, `Number of Articles`)
kable(title_topics)
...
The AFINN Lexicon assigns each word or term a value, or "sentiment score" from -5 to 5, with 5 being the most positive sentiment and -5 being the most negative sentiment. The table below shows that most terms in the articles are classified by the lexicon as neutral to slightly positive.  
